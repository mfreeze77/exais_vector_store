...
The skipped test is the live Postgres RLS integration suite. It is wired in `.github/workflows/ci.yml` under `postgres-rls-integration` and requires a Docker Compose Postgres service plus `SVS_RUN_INTEGRATION=1`.

## Remaining production integration

This is a production-candidate codebase. The remaining work is deployment-specific: provider credentials, Expert AI Services corpus bakeoffs, load tests against the target corpus, DNS/TLS/secrets manager wiring, and live backup/restore drills on the chosen VPS/dedicated-server topology.
...
Use `.env.example` for local developer cells. Use `.env.production.example` as the starting point for customer/micro-production cells. Production startup fails closed if dev mode is enabled, the API-key pepper is still the default, wildcard CORS is configured, or HTTPS OpenSearch certificate verification is disabled.
