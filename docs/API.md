...
```json
{
  ...
  "source_uri": "https://example.gov/budget/fy2027.pdf",
  "source_identity": "statecivics:logical-document-sha256",
  "security_level": 1,
...
`source_identity` is the connector's stable logical-document key. When present,
exact-byte deduplication is scoped to that identity and changed content versions
the same document even if its public `source_uri` moves. `source_uri` remains the
human citation URL; it is not a durable identity. Connectors should supply
`source_identity` whenever two distinct official documents could contain equal
bytes or a publisher may change URLs between revisions.
